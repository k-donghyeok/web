...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 친구 이름 설정 및 시간표 로드
    setFriendName(friendUid);
    loadUserTimetable(friendUid);
});

async function setFriendName(userId) {
    const userDoc = await getDoc(doc(db, 'users', userId));
    const userNameElement = document.getElementById('friend-name');
    if (userDoc.exists()) {
        const userData = userDoc.data();
        userNameElement.textContent = `${userData.name}의 시간표`;
    } else {
        userNameElement.textContent = '친구의 시간표';
    }
}
